...
import dev.Zerphyis.upload.repository.RepositoryImportacao;
import dev.Zerphyis.upload.repository.RepositoryTransacao;
...
public class TransacaoService {
    @Autowired
    RepositoryTransacao transacaoRepository;

    @Autowired
    RepositoryImportacao importacaoRepository;
    ...
    public void processarEImportarTransacoes(MultipartFile file) throws Exception {
        ...
        if (importacaoRepository.existsByDataTransacao(dataTransacao)) {
            throw new Exception("Transações para a data " + dataTransacao + " já foram importadas.");
        }
        ...
        try (Reader reader = new InputStreamReader(file.getInputStream())) {
            ...
            for (CSVRecord record : parser) {
                ...
                try {
                    ...
                    transacao.setDatatTransacao(LocalDateTime.parse(record.get(7).replace("T", " "), formatter));
                } catch (NumberFormatException | java.time.format.DateTimeParseException e) {
                    throw new Exception("Erro ao converter valor ou data na linha: " + record.getRecordNumber(), e);
                }
